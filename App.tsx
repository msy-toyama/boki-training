             ...
             <div className="space-y-4">
               <div className="flex gap-3">
                 <button onClick={() => {
                   ...
                   shareToX(text);
                 }} className="flex-1 py-3 bg-black hover:bg-slate-800 text-white font-bold rounded-full transition-colors flex items-center justify-center gap-2 shadow-lg">
                   <Twitter size={20} /> Xでポスト
                 </button>
                 <button onClick={() => {
                   audioService.playSfx(SoundType.SFX_SELECT);
                   const text = isClear 
                     ...
                     : `【${difficulty}モード】スコア: ${playerState.score.toLocaleString()}点 / 討伐数: ${monsterIndex}体 / 到達: ${questionsAnswered}問 - 簿記トレーニング大戦で学習中！`;
                   shareNative(text);
                 }} className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-full transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/50">
                   <Share2 size={20} /> その他
                 </button>
               </div>
               <button onClick={() => {
